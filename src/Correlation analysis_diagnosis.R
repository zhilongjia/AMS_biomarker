...
# select the q-value for ploting (BH)
...
write.csv(corr_results, file = "../results/diagnosis_3group_merge_qp0.05_corr_BH_results.csv")
...
write.table(data, file="../results/diagnosis_3feature_merge_inputada_BH_p0.05.txt", sep="\t", row.names=F, quote=F)

net_data <- data %>% subset(select = -c(p_value))
net_data[,'r'] <- abs(net_data[,'r'])

write.table(net_data, file="../results/diagnosis_3feature_merge_input_network_BH_p0.05.txt", sep="\t", row.names=F, quote=F)

save.image('diagnosis_BH_sig_corr.RData')
