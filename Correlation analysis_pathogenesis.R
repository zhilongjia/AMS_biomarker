...
comp_i <- c("AMS1k", "AMS4k")

####### data prepared #######
# differential clinical indexes and DEPs were selected for the correlation analysis
...
sub_clinic <- clin_raw_re %>% filter(q_value <0.05)
...
#remove phen 75% unrecorded unless they are involved in LLS
phen_data <- corr_raw %>%
...
all <- corr_raw  %>% tibble::column_to_rownames(var='sample_id') %>%
  ...
  bind_cols(t_phen_raw2) %>% select(-c(group))
...
#select p-value not q-value (not BH)
comp_list_low <- list( pd = c('p','d'),
                       pc = c('p','c'),
                       dc = c('c','d')                      
)
...
write.csv(corr_results, file = "../results/pathogenesis_3group_merge_q0.05_corr_notBH_results.csv")
...
write.table(data, file="../results/pathogenesis_3feature_merge_inputada_notBH_p0.05.txt", sep="\t", row.names=F, quote=F)

net_data <- data %>% subset(select = -c(p_value))
net_data[,'r'] <- abs(net_data[,'r'])

write.table(net_data, file="../results/pathogenesis_3feature_merge_input_network_notBH_p0.05.txt", sep="\t", row.names=F, quote=F)

# cytoscape input group
g1 <- data.frame(symbol,check.names=F) %>% mutate(group = "Proteins") %>%
  rename(feature = symbol) 
 
g2 <- data.frame(clinic,check.names=F)  %>% mutate(group = "Clinical Indexes") %>% 
  rename(feature = clinic) 
g3 <- data.frame(colnames(LLS),check.names=F)  %>% mutate(group = "AMS phenotypes") %>% 
  rename(feature = 'colnames(LLS)') %>% filter(feature != c('sample_id','group'))

net_group <- rbind(g1,g2,g3)
write.table(net_group, file="../results/cytoscape_input_group.txt", sep="\t", row.names=F, quote=F)
